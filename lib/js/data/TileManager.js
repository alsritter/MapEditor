...
class TileManager {
   ...
   */
  constructor(_ctx, cols, rows, _img2) {
    ...
    for (let col = 0; col < _classPrivateFieldGet(this, _cols); col++) {
      let temp = [];

      for (let row = 0; row < _classPrivateFieldGet(this, _rows); row++) {
        // 还要判断当前的图是否是空的
        let tempX = row * _classPrivateFieldGet(this, _sHeight);

        let tempY = col * _classPrivateFieldGet(this, _sWidth);
...
var _tileIsEmpty2 = function _tileIsEmpty2(ctx, img, x, y, space) {
  ...
  let imageData = ctx.getImageData(0, 0, space, space).data; // 只要超过 三分之一的内容就表示当前图像不是空的（因为是 RGBA 所以需要除 4）
